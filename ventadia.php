...
$sql2 = 'SELECT c.nombre cam, s.nombre sup, SUM(v.trx) trx, SUM(v.seg_cesantia) cesantia, SUM(v.seg_vida) vida, SUM(v.monto) total FROM venta v, supervisor s, campana c WHERE v.id_campana=c.id and v.id_supervisor=s.id and v.fecha=curdate() GROUP BY v.id_supervisor, v.id_campana order by sup';
$result2= $conn->prepare($sql2);
$result2->execute();
$cruce_venta=$result2->fetchall();

$sql3 = 'SELECT c.nombre nombre, SUM(v.trx) trx, SUM(v.seg_cesantia) cesantia, SUM(v.seg_vida) vida, SUM(v.monto) total FROM venta v, campana c where v.id_campana=c.id and v.fecha=curdate() GROUP BY v.id_campana ORDER BY total desc';
$result3=$conn->prepare($sql3);
$result3->execute();
$venta_campana=$result3->fetchall();

$sql4 = 'SELECT s.nombre nombre, SUM(v.trx) trx, SUM(v.seg_cesantia) cesantia, SUM(v.seg_vida) vida, SUM(v.monto) total FROM venta v, supervisor s where v.id_supervisor=s.id and v.fecha=curdate() GROUP BY v.id_supervisor order by total desc';
...
<html lang="es">
<head>
	<title>Ranking Acumulado dia</title>
	<meta charset="utf-8">
...
<body>
	...
	<div class="container-fluid text-center" style="margin-top: 51px">
      ...
      <h2>Ventas Acumuladas <?php echo date('d-m-Y');?> </h2>
        ...
        <div class="col-sm-12 tex-center">
            <h3>Ventas Supervisor X campaña <?php date("d-m-Y") ?></h3>
              <table class="table table-bordered">
                <thead>
                  <tr class="info">
                    <th>Supervisor</th>
